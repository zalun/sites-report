...
def config_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Write a minimal valid config and set required env vars."""
...
def test_db_status_shows_empty_tables(
    runner: CliRunner, config_path: Path, tmp_path: Path
) -> None:
    db_path = tmp_path / "data" / "sites-report.db"
    cfg_text = MINIMAL_CONFIG.replace(
    ...
    runner.invoke(cli, ["--config", str(config_path), "init"])
...
def test_db_status_shows_schema_version(
    runner: CliRunner, config_path: Path, tmp_path: Path
) -> None:
    db_path = tmp_path / "data" / "sites-report.db"
    cfg_text = MINIMAL_CONFIG.replace(
...
def test_db_status_fails_on_uninitialized_db(
    ...
    result = runner.invoke(cli, ["--config", str(config_path), "db-status"])
    assert result.exit_code == 1
    assert "database error" in result.stderr.lower()
